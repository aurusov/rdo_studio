...
LogStyle::LogStyle()
    ...
    , borders()
{
    fontStyle = StyleFont::Style::NONE;
...
bool LogStyle::getItemColors(const std::string& item, LogColorPair &colors) const
{
    ...
    const auto posstart = item.find_first_not_of(' ');
    const auto posend = item.find_first_of(' ', posstart);
    const auto key = boost::algorithm::trim_copy(item.substr(posstart, posend - posstart));
    bool res = true;
    if (key == "ES")
    {
        colors = es;
...
QSettings& operator<< (QSettings& settings, const LogStyle& style)
{
    ...
    settings.setValue("style", static_cast<int>(style.fontStyle));
...
QSettings& operator>> (QSettings& settings, LogStyle& style)
{
    ...
    settings.beginGroup("theme");
    style.fontStyle = static_cast<StyleFont::Style>(settings.value("style", static_cast<int>(style.fontStyle)).toInt());
