...
double RDOPatternEvent::getNextTimeInterval(CREF(LPRDORuntime) pRuntime)
{
	...
	pRuntime->error().push(rdo::format(_T("Попытка запланировать событие в прошлом. Выражение времени для $Time имеет отрицательное значение: %f"), time_next), m_timeCalc);
	return 0;
}
...
double RDOPatternOperation::getNextTimeInterval(CREF(LPRDORuntime) pRuntime)
{
	double time_next = m_timeCalc->calcValue(pRuntime).getDouble();
	if (time_next >= 0) return time_next;
	pRuntime->error().push(rdo::format(_T("Попытка запланировать окончание операции в прошлом. Выражение времени для $Time имеет отрицательное значение: %f"), time_next), m_timeCalc);
	return 0;
}
