...
REF(RDOValue) RDOCalcCheckRange::doCalc(CREF(LPRDORuntime) pRuntime)
{
	...
	{
		pRuntime->error().push(rdo::format(_T("Значение выходит за допустимый диапазон [%s..%s]: %s"), m_minValue.getAsString().c_str(), m_maxValue.getAsString().c_str(), m_value.getAsString().c_str()), this);
	}
	return m_value;
}
