...
// --------------------------------------------------------------------------------
dptrtp_main:
	| dptrtp_main RDO_Decision_point error RDO_End /* Ð·Ð°Ð³Ð»ÑƒÑˆÐºÐ° Ð´Ð»Ñ $Decision_point */
	| dptrtp_main RDO_Activities error RDO_End     /* Ð·Ð°Ð³Ð»ÑƒÑˆÐºÐ° Ð´Ð»Ñ $Activities     */
...
dpt_process_line
	...
	| RDO_QUEUE error
	{
		CONVERTER->error().error(@1, rdo::format(_T("ÐžÐ¶Ð¸Ð´Ð°ÐµÑ‚ÑÑ Ð¸Ð¼Ñ Ñ€ÐµÑÑƒÑ€ÑÐ° Ð´Ð»Ñ ÑÐ±Ð¾Ñ€Ð° ÑÑ‚Ð°Ñ‚Ð¸ÑÑ‚Ð¸ÐºÐ¸ Ð¿Ð¾ Ð¾Ñ‡ÐµÑ€ÐµÐ´Ð¸")));
	}
	...
	| RDO_SEIZE error
	{
		CONVERTER->error().error(@1, rdo::format(_T("ÐžÐ¶Ð¸Ð´Ð°ÐµÑ‚ÑÑ ÑÐ¿Ð¸ÑÐ¾Ðº Ñ€ÐµÑÑƒÑ€ÑÐ¾Ð², Ð¾Ð±ÑŠÐµÐ´Ð¸Ð½ÑÐµÐ¼Ñ‹Ñ… Ð² Ð±Ð»Ð¾Ðº, Ñ‡ÐµÑ€ÐµÐ· Ð·Ð°Ð¿ÑÑ‚ÑƒÑŽ")));
	}
...
dpt_queue_param
	...
	| RDO_IDENTIF error
	{
		CONVERTER->error().error(@1, _T("ÐžÑˆÐ¸Ð±ÐºÐ° Ð² Ð¸Ð¼ÐµÐ½Ð¸ Ñ€ÐµÑÑƒÑ€ÑÐ°"));
	}
	;
...
// --------------------------------------------------------------------------------
dptrtp_main:
	| dptrtp_main RDO_Decision_point error RDO_End /* çàãëóøêà äëÿ $Decision_point */
	| dptrtp_main RDO_Activities error RDO_End     /* çàãëóøêà äëÿ $Activities     */
	| dptrtp_main dpt_process_end
	;
...
dpt_process_line
	...
	| RDO_QUEUE error
	{
		CONVERTER->error().error(@1, rdo::format(_T("Îæèäàåòñÿ èìÿ ðåñóðñà äëÿ ñáîðà ñòàòèñòèêè ïî î÷åðåäè")));
	}
	| RDO_DEPART dpt_depart_param
	| RDO_DEPART error
	{
	...
	| RDO_SEIZE error
	{
		CONVERTER->error().error(@1, rdo::format(_T("Îæèäàåòñÿ ñïèñîê ðåñóðñîâ, îáúåäèíÿåìûõ â áëîê, ÷åðåç çàïÿòóþ")));
	}
	| RDO_RELEASE dpt_release_param
	| RDO_RELEASE error
	{
...
dpt_queue_param
	...
	| RDO_IDENTIF error
	{
		CONVERTER->error().error(@1, _T("Îøèáêà â èìåíè ðåñóðñà"));
	}
	;
