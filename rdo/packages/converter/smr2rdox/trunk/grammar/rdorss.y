...
rss_res_type
	: RDO_IDENTIF_COLON RDO_IDENTIF
	{
		...
		if (!pResType)
		{
			CONVERTER->error().error(@2, rdo::format(_T("ÐÐµÐ¸Ð·Ð²ÐµÑÑ‚Ð½Ñ‹Ð¹ Ñ‚Ð¸Ð¿ Ñ€ÐµÑÑƒÑ€ÑÐ°: %s"), type->value().getIdentificator().c_str()));
		}
		LPRDORSSResource pResourceExist = CONVERTER->findRSSResource(name->value().getIdentificator());
		if (pResourceExist)
		{
			CONVERTER->error().push_only(name->src_info(), rdo::format(_T("Ð ÐµÑÑƒÑ€Ñ '%s' ÑƒÐ¶Ðµ ÑÑƒÑ‰ÐµÑÑ‚Ð²ÑƒÐµÑ‚"), name->value().getIdentificator().c_str()));
...
rss_res_type
	: RDO_IDENTIF_COLON RDO_IDENTIF
	{
		...
		if (!pResType)
		{
			CONVERTER->error().error(@2, rdo::format(_T("Íåèçâåñòíûé òèï ðåñóðñà: %s"), type->value().getIdentificator().c_str()));
		}
		LPRDORSSResource pResourceExist = CONVERTER->findRSSResource(name->value().getIdentificator());
		if (pResourceExist)
		{
			CONVERTER->error().push_only(name->src_info(), rdo::format(_T("Ðåñóðñ '%s' óæå ñóùåñòâóåò"), name->value().getIdentificator().c_str()));
