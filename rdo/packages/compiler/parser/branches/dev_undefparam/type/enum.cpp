...
LPRDOType RDOEnumType::type_cast(CREF(LPRDOType) from, CREF(RDOParserSrcInfo) from_src_info, CREF(RDOParserSrcInfo) to_src_info, CREF(RDOParserSrcInfo) src_info) const
{
	switch (from->type()->typeID())
	{
		case rdoRuntime::RDOType__int::t_enum:
		{
			...
			{
				rdoParser::g_error().push_only(src_info,     _T("Используются различные перечислимые типы с одинаковыми значениями"));
				...
				rdoParser::g_error().push_only(src_info,     _T("Возможно, удобнее использовать первый из них как перечислимый, а второй как such_as на него, тогда параметры можно будет сравнивать и присваивать"));
			}
			else
			{
				rdoParser::g_error().push_only(src_info,     _T("Несоответствие перечислимых типов"));
				rdoParser::g_error().push_only(to_src_info,   to_src_info.src_text());
				rdoParser::g_error().push_only(src_info,     _T("и"));
				rdoParser::g_error().push_only(from_src_info, from_src_info.src_text());
			}
		...
		case rdoRuntime::RDOType::t_identificator:
		{
			...
			rdoParser::g_error().error(src_info, rdo::format(_T("Значение '%s' не является элементом перечислимого типа %s"), from_src_info.src_text().c_str(), to_src_info.src_text().c_str()));
			break;
		}
		default:
			rdoParser::g_error().push_only(src_info,    rdo::format(_T("Ожидается значение перечислимого типа, найдено: %s"), from_src_info.src_text().c_str()));
...
LPRDOValue RDOEnumType::value_cast(CREF(LPRDOValue) pFrom, CREF(RDOParserSrcInfo) to_src_info, CREF(RDOParserSrcInfo) src_info) const
{
	...
	{
		rdoParser::g_error().push_only(src_info,    rdo::format(_T("Неверное значение параметра перечислимого типа: %s"), pFrom->src_info().src_text().c_str()));
		rdoParser::g_error().push_only(to_src_info, rdo::format(_T("Возможные значения: %s"), name().c_str()));
		rdoParser::g_error().push_done();
	}
	return pToValue;
}
...
void RDOEnumType::add(CREF(LPRDOValue) pNext)
{
	...
	{
		rdoParser::g_error().error(pNext->src_info(), rdo::format(_T("Значение перечислимого типа уже существует: %s"), pNext->src_text().c_str()));
	}
