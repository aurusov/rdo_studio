/*!
  ...
  \file      rdoprocess_method_manager.cpp
			...
			if ( finder.IsDirectory() && !finder.IsDots() ) {
				...
				if ( fullname.rfind( ".dll" ) == fullname.length() - 4 ) {
					...
					if ( RPMethodPlugin::isMethod( modulName ) ) {
						...
						listctrl->Create( WS_CHILD | LVS_SORTASCENDING | LVS_AUTOARRANGE | LVS_ICON | LVS_SINGLESEL | LVS_NOLABELWRAP, CRect(0,0,1,1), studioApp.getIMainWnd()->workspace.pagectrl->prepareNewPage(), 1 );
						...
						RPPageCtrlItem* page = studioApp.getIMainWnd()->workspace.pagectrl->insertPage( listctrl, plugin->getMethod()->getName() );
...
void RPMethodManager::insertMethod( rpMethod::RPMethod* method )
{
	...
	RPPageCtrlItem* page = studioApp.getIMainWnd()->workspace.pagectrl->insertPage( listctrl, method->getName() );
