...
{
	// ѕереходим к первому потомку узла Node, который установлен на <Model>:
	pugi::xml_node node = Node.first_child();
	// ѕровер€ем, а стоим ли мы на узле RPProjectMFC ?
	if(strcmp(node.name(), getClassName().c_str()) == 0)
	{
		std::list<PTR(RPObject)> mfchild;
		getAllChild(mfchild);
		// ƒл€ каждого FlowChart'а вызываем его функцию loadFromXML(node), где "node" есть <RPProjectMFC>
