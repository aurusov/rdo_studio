...
public:
	...
	double generate()
	{
		...
		double h  = 2/(m_max-m_min);					//высота треугольника
		double a1 = h/(m_mean-m_min);					//параметры уравнения левой части
		double b1 = h;
		double a2 = -h/(m_max-m_mean);					//параметры уравнения правой части
		double b2 = h;
		if ( u01 >= (m_mean-m_min)/(m_max-m_min))
		{
			result = (generate01()*h - b2)/a2;			//домножение на h - масштабирование с целью попасть в область определения исходной функции, она же область допустимых значений аргумента для обратной функции
		}
		else
		{
			result = (generate01()*h - b1)/a1;
		}					
