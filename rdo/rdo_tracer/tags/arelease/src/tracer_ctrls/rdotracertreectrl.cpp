...
#include "../trace_files/rdotracerrestype.h"
//#include "../rdotracerresource.h"
#include "../trace_files/rdotracerresource.h"
//#include "../rdotracerpattern.h"
#include "../trace_files/rdotracerpattern.h"
//#include "../rdotraceroperation.h"
#include "../trace_files/rdotraceroperation.h"
//#include "../rdotracerresult.h"
#include "../trace_files/rdotracerresult.h"
#include "../../resource.h"
...
int RDOTracerTreeCtrl::OnCreate( LPCREATESTRUCT lpCreateStruct )
{
	...
	rootItem.setTreeItem( InsertItem( format( ID_MODEL_START ).c_str(), 0, 0 ) );
	rtpItem.setTreeItem( InsertItem( format( ID_RESOURCE_TYPES ).c_str(), 1, 1, rootItem.getTreeItem() ) );
	patItem.setTreeItem( InsertItem( format( ID_PATTERNS ).c_str(), 1, 1, rootItem.getTreeItem() ) );
	pmvItem.setTreeItem( InsertItem( format( ID_RESULTS ).c_str(), 1, 1, rootItem.getTreeItem() ) );
...
{
	return SetItemText( rootItem.getTreeItem(), format( ID_MODEL, modelName.c_str() ).c_str() );
}
...
void RDOTracerTreeCtrl::clear()
{
	...
	SetItemText( rootItem.getTreeItem(), format( ID_MODEL_START ).c_str() );
}
