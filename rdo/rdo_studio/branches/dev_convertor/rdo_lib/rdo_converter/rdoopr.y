...
opr_header
	: RDO_Operations
	{
		...
		ASSERT(pOperations);
		$$ = CONVERTOR->stack().push(pOperations);
	}
	;
...
opr_body
	: opr_header RDO_IDENTIF_COLON RDO_IDENTIF
	{
		...
		ASSERT(pOperation);
		$$ = CONVERTOR->stack().push(pOperation);
	}
...
// ----------------------------------------------------------------------------
fun_arithm
	: RDO_INT_CONST                      { $$ = (int)new RDOFUNArithm(CONVERTER, RDOVALUE($1)); }
	| RDO_REAL_CONST                     { $$ = (int)new RDOFUNArithm(CONVERTER, RDOVALUE($1)); }
	| RDO_BOOL_CONST                     { $$ = (int)new RDOFUNArithm(CONVERTER, RDOVALUE($1)); }
	| RDO_STRING_CONST                   { $$ = (int)new RDOFUNArithm(CONVERTER, RDOVALUE($1)); }
	| RDO_IDENTIF                        { $$ = (int)new RDOFUNArithm(CONVERTER, RDOVALUE($1)); }
	| RDO_IDENTIF '.' RDO_IDENTIF        { $$ = (int)new RDOFUNArithm(CONVERTER, RDOVALUE($1), RDOVALUE($3)); }
	| RDO_IDENTIF_RELRES '.' RDO_IDENTIF { $$ = (int)new RDOFUNArithm(CONVERTER, RDOVALUE($1), RDOVALUE($3)); }
	| fun_arithm '+' fun_arithm		     { $$ = (int)(ARITHM($1) + ARITHM($3)); }
	| fun_arithm '-' fun_arithm		     { $$ = (int)(ARITHM($1) - ARITHM($3)); }
	| fun_arithm '*' fun_arithm		     { $$ = (int)(ARITHM($1) * ARITHM($3)); }
	| fun_arithm '/' fun_arithm		     { $$ = (int)(ARITHM($1) / ARITHM($3)); }
	...
	| '-' fun_arithm %prec RDO_UMINUS
	{
		...
		$$ = (int)new RDOFUNArithm(CONVERTER, RDOValue(ARITHM($2).type(), info), rdo::Factory<rdoRuntime::RDOCalcUMinus>::create(ARITHM($2).createCalc()));
	}
	;
...
fun_arithm_func_call
	: RDO_IDENTIF '(' ')' 
	{
		PTR(RDOFUNParams) fun = new RDOFUNParams(CONVERTER);
...
fun_group_header
	: fun_group_keyword '(' RDO_IDENTIF_COLON
	{
		...
		$$ = (int)(new RDOFUNGroupLogic(CONVERTER, (RDOFUNGroupLogic::FunGroupType)$1, type_name->src_info()));
	}
...
fun_select_header
	...
	{
		PTR(RDOValue) type_name  = P_RDOVALUE($3);
		PTR(RDOFUNSelect) select = new RDOFUNSelect(CONVERTER, type_name->src_info());
