...
stopping_statement
	: RDO_IDENTIF '.' RDO_Stopping '(' ')' ';'
	{
		...
		if (!pEvent)
		{
			CONVERTER->error().error(@1, rdo::format(_T("Попытка остановить неизвестное событие: %s"), eventName.c_str()));
		}
...
planning_statement
	...
	| RDO_IDENTIF '.' RDO_Start '(' event_descr_param ')' ';'
	{
		...
		if (!pEvent)
		{
			CONVERTER->error().error(@1, rdo::format(_T("Попытка запустить неизвестное событие: %s"), eventName.c_str()));
		}
