...
smr_model
	: RDO_Model_name '=' RDO_IDENTIF 
	{
		...
		ASSERT(pSMR);
		$$ = PARSER->stack().push(pSMR);
	}
	| RDO_Model_name '=' error
	{
		PARSER->error().error( @2, @3, "Ожидается имя модели" );
	}
	| RDO_Model_name error
	{
...
smr_descr
	...
	{
		PARSER->error().error( @3, @4, "Ожидается имя файла ресурсов" );
	}
	| smr_descr RDO_Resource_file error
	{
	...
	{
		PARSER->error().error( @3, @4, "Ожидается имя файла операций" );
	}
	| smr_descr RDO_OprIev_file error
	{
		PARSER->error().error( @2, "Ожидается '='" );
	...
	{
		PARSER->error().error( @3, @4, "Ожидается имя файла анимации" );
	}
	| smr_descr RDO_Frame_file error
	{
		PARSER->error().error( @2, "Ожидается '='" );
	...
	{
		PARSER->error().error( @3, @4, "Ожидается имя файла собираемых показателей" );
	}
	| smr_descr RDO_Statistic_file error
	{
		PARSER->error().error( @2, "Ожидается '='" );
	...
	{
		PARSER->error().error( @3, @4, "Ожидается имя файла результатов" );
	}
	| smr_descr RDO_Results_file error
	{
		PARSER->error().error( @2, "Ожидается '='" );
	...
	{
		PARSER->error().error( @3, @4, "Ожидается имя файла трассировки" );
	}
	| smr_descr RDO_Trace_file error
	{
		PARSER->error().error( @2, "Ожидается '='" );
