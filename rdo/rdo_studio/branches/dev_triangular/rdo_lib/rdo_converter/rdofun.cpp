	...
	: RDOFUNSequence(pHeader, seed)
{
	if (m_pHeader->getType()->type()->typeID() == rdoRuntime::RDOType::t_enum)
	{
		Converter::s_converter()->error().error(src_info(), _T("Последовательность типа '%s' не может возвращять значения перечислимого типа"));
	}
}
...
LPRDOFUNArithm RDOFUNSequenceTriangular::createCallCalc(REF(LPRDOFUNParams) pParamList, CREF(RDOParserSrcInfo) seq_src_info) const
{
	...
	{
		Converter::s_converter()->error().error(seq_src_info, rdo::format(_T("Для треугольного закона распределения '%s' нужно указать два параметра: левую границу, математическое ожидание, правую границу"), name().c_str()));
	}
