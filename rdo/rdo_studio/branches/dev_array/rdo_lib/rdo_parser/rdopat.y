...
member_statement_parent
	: RDO_IDENTIF
	{
		...
		LPContext pContext = pCurrentContext->find(RDOVALUE($1));
		if (!pContext)
		{
			PARSER->error().error(@1, rdo::format(_T("Неизвестный идентификатор: %s"), RDOVALUE($1)->getIdentificator().c_str()));
		}
...
set_array_item_statement
	: RDO_IDENTIF '[' fun_arithm ']' '=' fun_arithm
	{
		...
		rdoRuntime::LPRDOCalc pCalc = rdo::Factory<rdoRuntime::RDOCalcSetArrayItem>::create(pArrayArithm->calc(), pArithmInd->calc(), pRightArithm->calc());
		ASSERT(pCalc);
