...
opr_main
	...
	| error
	{
		...
		{
			PARSER->error().error( @1, "Ожидается ключевое слово $Operations" );
		}
		else
		{
...
opr_prior
	...
	| RDO_Priority fun_arithm opr_header
	{
		...
		{
			PARSER->error().error(@3, _T("Операции пока не могут иметь приоритет"));
		}
	}
...
opr_body
	: opr_prior RDO_IDENTIF_COLON RDO_IDENTIF
	{
	...
	{
		PARSER->error().error( @2, @3, "Ожидается имя образца" );
	}
	...
	| opr_prior error
	{
		PARSER->error().error( @2, "Ожидается имя операции" );
	}
	;
...
opr_param
	: opr_param  '*'              {reinterpret_cast<RDOOPROperation*>($1)->addParam(RDOValue(RDOParserSrcInfo(@2, "*")));}
