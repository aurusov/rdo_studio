...
inline void RDOTypeRangeRange::checkRange() const throw(...)
{
	...
	{
		rdoParse::g_error().error(m_max_value.src_info(), rdo::format(_T("Границы диапазона должны быть одного типа, найдено: [%s] .. [%s]"), m_min_value.type()->name().c_str(), m_max_value.type()->name().c_str()));
	}
...
{
	return rdo::Factory<rdoRuntime::RDOCalcCheckDiap>::create(range()->getMin().value(), range()->getMax().value(), T::calc_cast(pCalc, pType)).object_cast<rdoRuntime::RDOCalc>();
}
...
{
	return RDOValue(m_range->getMin().value(), rdo::Factory<RDOType__int>::create().object_cast<RDOType>(), RDOParserSrcInfo());
}

template<>
inline RDOValue RDOTypeRange<RDOType__real>::get_default() const
{
	return RDOValue(m_range->getMin().value(), rdo::Factory<RDOType__real>::create().object_cast<RDOType>(), RDOParserSrcInfo());
}
