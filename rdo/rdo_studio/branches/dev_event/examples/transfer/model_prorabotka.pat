...
заявка
    Convert_event trace
		...
		вес_груза    = 6000.0;//ф_вес_груза (900.0, 3600.0);
		маршрут      = 1;//ф_тип_маршрута;
$End
...
заявка
	...
	Convert_rule
		if (Int(заявка.вес_груза/тоннаж) < заявка.вес_груза/тоннаж)
			количество_вагонов_в_заявке = Int(заявка.вес_груза/тоннаж) + 1;
		else
			количество_вагонов_в_заявке = Int(заявка.вес_груза/тоннаж);
...
формируемый_состав
	...
	Convert_rule
		if (текущее_количество_вагонов == вагоны_в_составе)
		{
			полный            = true;
			состояние         = заполнен;
//			заявки_в_составе -= 1;            //не пойму, зачем здесь уменьшать число заявок
		}
		else
			состояние = распределение_вагонов;
$End
...
$Pattern Образец_создания_реального_состава: rule
$Relevant_resources
    формируемый_состав: формируемые_составы Keep
	_парк             : парк                Keep 
