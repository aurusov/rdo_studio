...
void print_RTP(REF(rdoConverter::RDOCorba::GetRTP_var) my_rtpList)
{
	PTR(FILE) f1;
#pragma warning(disable: 4996)
	f1 = fopen(file1, _T("w"));
#pragma warning(default: 4996)
	...
	for (ruint i = 0; i < my_rtpList->length(); i++)
	{
		...
		if (my_rtpList[i].m_type==rdoConverter::RDOCorba::rt_permanent)
		...
		for (ruint j = 0; j != my_rtpList[i].m_param_count; j++)
		{
			...
			{
			case rdoConverter::RDOCorba::int_type:
				...
				break;
			case rdoConverter::RDOCorba::double_type:
					...
					break;
				case rdoConverter::RDOCorba::enum_type:
				{
...
void print_RSS(REF(rdoConverter::RDOCorba::GetRSS_var) my_rssList)
{
	PTR(FILE) f2;
#pragma warning(disable: 4996)
	f2 = fopen(file2, _T("w"));
...
//! ---------- RDOParserCorbaRTP
//! ----------------------------------------------------------------------------
void RDOParserCorbaRTP::parse(PTR(Converter) pParser)
{
	...
	rdoConverter::RDOParserSMRInfo parser;
	parser.parse();

	CPTR(char) left;
	CPTR(char) right;

	rdoConverter::RDOSMR::StringTable tmp = parser.getSMR()->getExternalModelList();
	STL_FOR_ALL_CONST(rdoConverter::RDOSMR::StringTable, tmp, it)
	{
		left  = it->first.c_str();
		...
		try
		{
			...
			rdoConverter::RDOCorba_var rdocorbaref = rdoConverter::RDOCorba::_narrow(obj);

			//*************************************************************
			CORBA::Long rtp_count = 0;

			rdoConverter::RDOCorba::GetRTP_var tmp_rtp = rdocorbaref->getRDORTPlist(rtp_count);
			rdoConverter::RDOCorba::GetRTP_var my_rtpList(tmp_rtp);
			...
			rdoConverter::RDOCorba::GetRSS_var tmp_rss = rdocorbaref->getRDORSSPlist(rss_count);
			rdoConverter::RDOCorba::GetRSS_var my_rssList(tmp_rss);
			...
			for (ruint i = 0; i < my_rssList->length(); i++)
			{
				...
				if (rssList.append<rdoConverter::RDORSSResource>(rss))
				{
					//! Добавили успешно
...
//! ---------- RDOParserCorbaRSS
//! ----------------------------------------------------------------------------
void RDOParserCorbaRSS::parse(PTR(Converter) pParser)
{
/*
