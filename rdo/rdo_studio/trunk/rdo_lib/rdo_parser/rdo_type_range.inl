...
inline void RDOTypeRangeRange::checkRange() const throw(...)
{
	...
	{
		rdoParse::g_error().error(m_max_value.src_info(), rdo::format(_T("Границы диапазона должна быть одного типа: %s .. %s"), m_min_value.type()->name().c_str(), m_max_value.type()->name().c_str()));
	}
