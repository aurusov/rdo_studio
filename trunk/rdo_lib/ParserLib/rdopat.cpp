...
void RDOPATPattern::res_insert( RDORelevantResource* res )
{
	if ( res->isDirect() ) {
		...
		if ( std::find( currParser->tmp_rss.begin(), currParser->tmp_rss.end(), rss ) != currParser->tmp_rss.end() ) {
			currParser->error( rdo::format( "Ресурс не может дважды использовать в качестве релевантного: %s", rss->getName()->c_str() ) );
		}
		...
		if ( rtp->isPermanent() ) {
			...
			if ( rel_cnt >= rss_cnt ) {
				currParser->error( rdo::format( "Не хватает свободных ресурсов типа: %s", rtp->getName()->c_str() ) );
			}
		}
	}
...
//				currParser->error("Wrong second converter status for resource of permanent type");
			...
			if ( end == CS_Create || end == CS_Erase || end == CS_NonExist ) {
				currParser->error( rdo::format("Недопустимый статус конвертора конца для постоянного ресурса: %s", RDOPATPattern::StatusToStr(end).c_str()) );
			}
		}
