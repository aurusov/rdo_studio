...
void RDOActivity::setPatternParameters(RDOSimulator *sim) 
{
	...
	{
		m_paramsCalcs.at(i)->calcValueBase( runtime );
	}
}

void RDOActivity::getRelevantResources( RDOSimulator* sim, std::list< RDOResourceTrace* >& rel_res_list )
{
	rel_res_list.clear();
...
void RDOActivity::updateConvertStatus( RDOSimulator* sim, const std::vector< RDOResourceTrace::ConvertStatus >& status_list )
{
	updateRelRes( sim );
	int i = 0;
	std::list< RDOResourceTrace* >::iterator it = m_relevantResources.begin();
...
{
	std::string res;
	for ( std::list< RDOResourceTrace* >::const_iterator i = m_relevantResources.begin(); i != m_relevantResources.end(); i++ ) {
		if ( *i ) {
...
{ 
	...
	if ( haveAdditionalCondition ) {
		if ( !additionalCondition->calcValueBase( runtime ).getBool() ) {
			return false;
		}
	}
