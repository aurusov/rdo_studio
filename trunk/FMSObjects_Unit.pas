...
type
  ...
  {$WARNINGS OFF}
    ...
    procedure ReadPicture(Section: string);
...
procedure TFMSObject.ReadPicture(Section: string);
var
  s, fileName: string;
  i, j: integer;
...
begin
  try
    with FMSForm do
    begin
      s := PlaneFile.ReadString(Section, 'picture', '');
      fileName := s;
      if ExtractFileDrive(s) = '' then fileName := GetNormalDir(PlaneFileName) + s;
      if not FileExists(fileName) then fileName := GetNormalDir(PlaneFileName) + 'DBBitmaps\' + s;
      if FileExists(fileName)     then
      begin
        FMSBitmap.LoadFromFile(fileName);
