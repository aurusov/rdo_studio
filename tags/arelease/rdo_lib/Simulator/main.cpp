...
{
	cout << "Usage: " + string(progName) + " <file_name.smr>\nPress any key ...";
	getch();
	return -1;
}
...
/////////////////   SMR file first part //////////////////////////////////
		...
		try {
			...
			cout << ex.getType().c_str() << " : " << ex.mess.c_str() << " in the first part of " + smrFileName + " (SMR) file\n" << flush;
			throw;
		}
		smrFile1.close();
...
/////////////////   RTP file //////////////////////////////////
		...
		try {
			...
			cout << ex.getType().c_str() << " : " << ex.mess.c_str() << " in " + rtpFileName + " (RTP) file\n" << flush;
			throw;
		}
		rtpFile.close();
...
/////////////////   RSS file //////////////////////////////////
		...
		try {
			...
			cout << ex.getType().c_str() << " : " << ex.mess.c_str() << " in " + rssFileName + " (RSS) file\n" << flush;
			throw;
		}
		rssFile.close();
...
/////////////////   FUN file //////////////////////////////////
		...
		if(funFile.is_open()) 
		{
			try {
				...
				cout << ex.getType().c_str() << " : " << ex.mess.c_str() << " in " << funFileName << " (FUN) file\n" << flush;
				throw;
			}
			funFile.close();
		}
...
/////////////////   PAT file //////////////////////////////////
		...
		try {
			...
			cout << ex.getType().c_str() << " : " << ex.mess.c_str() << " in " + patFileName + " (PAT) file\n" << flush;
			throw;
		}
		patFile.close();
...
/////////////////   OPR file //////////////////////////////////
			...
			try {
				...
				cout << ex.getType().c_str() << " : " << ex.mess.c_str() << " in " + oprFileName + " (OPR) file\n" << flush;
				throw;
			}
			oprFile.close();
		}
...
/////////////////   DPT file //////////////////////////////////
			...
			try {
				...
				cout << ex.getType().c_str() << " : " << ex.mess.c_str() << " in " + dptFileName + " (DPT) file\n" << flush;
				throw;
			}
			dptFile.close();
		}
...
/////////////////   PMD file //////////////////////////////////
		...
		if(pmdFile.is_open())
		{
			try {
				...
				cout << ex.getType().c_str() << " : " << ex.mess.c_str() << " in " << pmdFileName << " (PMD) file\n" << flush;
				throw;
			}
			pmdFile.close();
		}
...
/////////////////   SMR file second part //////////////////////////////////
		...
		try {
			...
			cout << ex.getType().c_str() << " : " << ex.mess.c_str() << " in the second part of " << smrFileName << " (SMR) file\n" << flush;
			throw;
		}
		smrFile2.close();

//		cout << "Found " << parser.allRTPResType.size() << " types \n" << flush;
