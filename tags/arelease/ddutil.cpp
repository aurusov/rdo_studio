...
//    dwStyle  = GetWindowStyle( wnd->m_hWnd );
//    dwStyle &= ~WS_POPUP;
...
{
	if ( parentWnd && parentWnd->GetSafeHwnd() ) {
		parentWnd->GetClientRect( &m_rcWindow );
		parentWnd->ClientToScreen( &m_rcWindow );
...
HRESULT CDisplay::InitClipper()
{
    ...
    pClipper->SetHWnd( 0, parentWnd->m_hWnd );
